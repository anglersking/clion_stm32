...
uint8_t IIC_Write_1Byte(uint8_t SlaveAddress, uint8_t REG_Address, uint8_t REG_data)
{
    ...
    if (HAL_I2C_Mem_Write(&hi2c1, SlaveAddress << 1, REG_Address, I2C_MEMADD_SIZE_8BIT, &REG_data, 1, 1000) != HAL_OK)
    {
...
uint8_t spl0601_read(uint8_t hwadr, uint8_t regadr)
{
    ...
    HAL_I2C_Mem_Read(&hi2c1, hwadr << 1, regadr, I2C_MEMADD_SIZE_8BIT, &reg_data, 1, 1000);
//    printf("%d",reg_data);
    return reg_data;
}
...
void spl06_pressure_rate_config(uint8_t background_rate,uint8_t oversamply)
{
    ...
    {
        uint8_t data;
        data = spl0601_read(HW_ADR, CFG_REG);//读取原寄存器值
        data |= 0X04;//P-SHIFT位置1
        spl0601_write(HW_ADR, CFG_REG, data);//重新写回寄存器					
    }
    switch(oversamply)
    {
    ...
    spl0601_write(HW_ADR, PRS_CFG_REG, data);//写入配置
}
...
void spl06_temperature_rate_config(uint8_t background_rate,uint8_t oversamply,uint8_t ext)
{
    ...
    spl0601_write(HW_ADR, TMP_CFG_REG, data);//写入配置
}
/***********************************************************************
 * 传感器测量模式配置与状态读取
 * @param[in] 
...
uint8_t spl06_get_chip_id(void)//获取产品ID//获取产品版本//由于版本在不同的传感器有不同，本历程只判断ID来识别SPL06
{
...
uint8_t spl0601_init(void)
{
    ...
    printf("ididiididid %x %x\n", spl06.chip_id&0xf0,PRODUCT_ID);
